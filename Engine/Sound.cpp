...
SoundSystem::SoundSystem()
	...
	static_assert(nChannelsPerSound <= XAUDIO2_MAX_AUDIO_CHANNELS,
		"WAVE File Format Error: Maximum audio channels per audio stream exceeded");
	static_assert(nSamplesPerSec >= XAUDIO2_MIN_SAMPLE_RATE,
		"WAVE File Format Error: Sample rate lower than minimum allowed");
	static_assert(nSamplesPerSec <= XAUDIO2_MAX_SAMPLE_RATE,
		"WAVE File Format Error: Sample rate exceeds maximum allowed");
...
Sound::Sound(const std::wstring& fileName, bool loopingWithAutoCueDetect)
	:
	Sound(fileName, loopingWithAutoCueDetect ? LoopType::AutoEmbeddedCuePoints : LoopType::NotLooping) {
}
...
SoundSystem::FileException::FileException(const wchar_t* file, unsigned int line, const std::wstring& note,
