...
void Graphics::DrawSpriteTransparency(int x, int y, const Surface & s, Color chroma) {
	DrawSpriteTransparency(x, y, s.GetRect(), s, chroma);
}

void Graphics::DrawSpriteTransparency(int x, int y, const RectI & srcRect, const Surface & s, Color chroma) {
	DrawSpriteTransparency(x, y, srcRect, GetScreenRect(), s, chroma);
}

void Graphics::DrawSpriteTransparency(int x, int y, RectI srcRect, const RectI& clip, const Surface& s, Color chroma) {
	assert(srcRect.left >= 0);
	assert(srcRect.right <= s.GetWidth());
	assert(srcRect.top >= 0);
	assert(srcRect.bottom <= s.GetHeight());
	...
	for (int sy = srcRect.top; sy < srcRect.bottom; sy++) {
		for (int sx = srcRect.left; sx < srcRect.right; sx++) {
			...
			if (srcPixel != chroma) {
				...
				const int xOrig = x + sx - srcRect.left;
				const int yOrig = y + sy - srcRect.top;
				const Color destPixel = GetPixel(xOrig, yOrig);
				const Color blendPixel = {
					unsigned char((srcPixel.GetR() + destPixel.GetR()) / 2),
					unsigned char((srcPixel.GetG() + destPixel.GetG()) / 2),
					unsigned char((srcPixel.GetB() + destPixel.GetB()) / 2)
				};
				PutPixel(xOrig,yOrig, blendPixel);
...
void Graphics::DrawSpriteSubstitute(int x, int y, Color substitute, 
	const RectI& srcRect, const Surface& s, Color chroma)
{
	DrawSpriteSubstitute(x, y, substitute, srcRect, GetScreenRect(), s, chroma);
}

void Graphics::DrawSpriteSubstitute(int x, int y, Color substitute, RectI srcRect, 
	const RectI& clip, const Surface& s, Color chroma)
{
	assert(srcRect.left >= 0);
	assert(srcRect.right <= s.GetWidth());
	assert(srcRect.top >= 0);
